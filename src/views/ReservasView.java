...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		lblValorSimbolo.setBounds(65, 332, 17, 25);
...
//Activa el evento, después del usuario seleccionar las fechas se debe calcular el valor de la reserva
				...
				if (txtFechaE.getDate() == null && txtFechaS.getDate() == null) {
					...
					Date fEntrada = txtFechaE.getDate();
					Date fSalida = txtFechaS.getDate();


					LocalDate fEntradaFormat = LocalDate.parse(formatter.format(fEntrada));
					LocalDate fSalidaFormat = LocalDate.parse(formatter.format(fSalida));
					LocalDate fActual =LocalDate.parse(new SimpleDateFormat("yyyy-MM-dd").format(Calendar.getInstance().getTime()));

					
					Long dias = ChronoUnit.DAYS.between(fEntradaFormat, fSalidaFormat);
					Long fEntradaActual = ChronoUnit.DAYS.between(fActual, fEntradaFormat);
					Long fSalidaActual = ChronoUnit.DAYS.between(fActual, fSalidaFormat);
					

					
					if (dias > 0 && fEntradaActual >= 0 && fSalidaActual >= 0) {
						int valorDeReserva = (int) (dias * tarifa);
						txtValor.setText(String.valueOf(valorDeReserva));

						fechaEntrada = String.valueOf(fEntradaFormat);
						fechaSalida = String.valueOf(fSalidaFormat);
					} else {
						JOptionPane.showMessageDialog(null, "Fechas no correctas/Fechas menor a la actual.");
		...
		btnsiguiente.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (ReservasView.txtFechaE.getDate() != null && ReservasView.txtFechaS.getDate() != null) {
					...
					Reserva resarva = new Reserva(fechaEntrada, fechaSalida, valor, formaPago);
					
			
					//TODO: Aqui debe de estar el metodo de guardar de controller.		
					
					// guardando datos de la reserva.
					ReservaController reservaController = new ReservaController();
					reservaController.guardar(resarva);
					
					System.out.println(resarva.getId());
					
					
					// vista de registo pasandole el id de la reserva
					RegistroHuesped registro = new RegistroHuesped(resarva.getId());
